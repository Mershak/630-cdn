...
# Server's physical location (Ashburn, VA)
...
# Simulated content database with varying sizes
CONTENT_DB = {}

def initialize_content():
    """Initialize content with varying sizes based on content ID."""
    for i in range(100):
        if i < 10:  # Popular content (0-9): medium size
            size = random.randint(5000, 10000)
        elif i < 30:  # Semi-popular content (10-29): medium-large size
            size = random.randint(10000, 50000)
        elif i < 80:  # Regular content (30-79): varying sizes
            size = random.randint(1000, 100000)
        else:  # Large content (80-99): very large size
            size = random.randint(100000, 500000)
        
        CONTENT_DB[f"content_{i}"] = "x" * size

# Initialize content at startup
initialize_content()

def calculate_network_delay(client_lat: float, client_lon: float, content_size: int, is_edge_request: bool = False) -> float:
    ...
    # Add content size-based delay (simplified model)
    # Assume 10Gbps network = 1.25GB/s = 1250MB/s
    content_delay = content_size / (1250 * 1000 * 1000)  # Convert to seconds
    
    total_delay = propagation_delay + total_router_delay + base_latency + jitter + content_delay
    return total_delay
...
async def get_content(content_id: str, request: Request, client_lat: float, client_lon: float):
    ...
    network_delay = calculate_network_delay(client_lat, client_lon, len(content), is_edge_request)
