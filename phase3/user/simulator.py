...
def generate_request_sequence():
    """Generate a sequence of requests with controlled popularity distribution."""
    sequence = []
    
    # Popular content (60% of requests)
    # Each user will request these frequently
    popular_items = [f"content_{i}" for i in range(10)]  # content_0 to content_9
    
    # Semi-popular content (30% of requests)
    # These represent content that's moderately accessed
    semi_popular = [f"content_{i}" for i in range(10, 30)]  # content_10 to content_29
    
    # Rare content (10% of requests)
    # These represent long-tail content
    rare_items = [f"content_{i}" for i in range(30, 100)]  # content_30 to content_99
    
    # Build the sequence
    for _ in range(REQUESTS_PER_USER):
        r = random.random()
        if r < 0.6:  # 60% popular
            sequence.append(random.choice(popular_items))
        elif r < 0.9:  # 30% semi-popular
            sequence.append(random.choice(semi_popular))
        else:  # 10% rare
            sequence.append(random.choice(rare_items))
    
    return sequence

def generate_user_specific_sequence(user_id: int):
    """Generate a sequence specific to a user's location and behavior pattern."""
    base_sequence = []
    
    # Each user has 2-3 favorite items from the popular content
    # This simulates user-specific preferences while maintaining overall popularity
    user_favorites = [f"content_{i}" for i in range(user_id % 3, 10, 3)]
    
    # Build sequence with user preferences
    for i in range(REQUESTS_PER_USER):
        if i % 5 < 3:  # 60% favorite content
            base_sequence.append(random.choice(user_favorites))
        elif i % 5 == 3:  # 20% semi-popular
            base_sequence.append(f"content_{random.randint(10, 29)}")
        else:  # 20% other (mix of popular and rare)
            if random.random() < 0.5:
                base_sequence.append(f"content_{random.randint(0, 9)}")  # popular
            else:
                base_sequence.append(f"content_{random.randint(30, 99)}")  # rare
...
class User:
    def __init__(self, user_data: dict):
        ...
        self.cache_hits = 0
        self.request_sequence = generate_user_specific_sequence(self.id)
        self.results = []
        self.edge_node = None
...
async def main():
    ...
    csv_path = RESULTS_DIR / f"phase3_results_{timestamp}.csv"
